...
router.post('/network', bodyParser(), async (ctx, next) => {
    ctx.status = 200;
    ...
    for (let i = 0; i < ips.length; i++) {
        ...
        urls.push({"string": url, "qr": qr});
    }
    // console.log(urls);
    ctx.body = {"urls": urls};
});

router.post('/qr', bodyParser(), async (ctx, next) => {
    ctx.status = 200;
    ctx.type = "application/json";
    var text = ctx.request.body.text || "";
    // console.log(text);
    if(text){
        ctx.body = await QRCode.toDataURL(text);
        ctx.body = {"qr": ctx.body}
    }
    // console.log(`"${text}" = "${ctx.body.qr}"`);
});
